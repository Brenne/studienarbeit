...
public class CommentsActivity extends Activity implements
		CompaniesFragment.OnCompanySelectedLitener, CommentsFragment.CommentsFragmentListeners, CommentFragment.OnRemoveFragmentListener {
	...
	public void onCompanySelected(Company company) {
		...
		transaction.replace(R.id.comments_fragment_container, commentFragment, TAG_COMMENT_FRAGMENT).commit();
	...
	public void addComment(List<Comment> existingComments) {	
		...
		FragmentTransaction transaction = getFragmentManager().beginTransaction();
		CompaniesFragment companiesFragment =CompaniesFragment.newCompaniesFragmentInstance(allCompanies);
		transaction.addToBackStack(null);
