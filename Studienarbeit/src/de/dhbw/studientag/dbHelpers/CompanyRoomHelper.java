...
//			 b._id AND b.shortName=?
			String query = "SELECT c."+MySQLiteHelper.ID+" AS " + COMPANYROOM_COMPANY_ID
					+ ", r."+MySQLiteHelper.ID+" AS " + COMPANYROOM_ROOM_ID + " FROM "
...
//			Log.d("studientag", query);
	...
	public static CompanyLocation getLocationByCompanyId(long id,
		...
		String query = "SELECT  b." 
				+ MySQLiteHelper.ID + "" + " AS "
				+ BUILDING_ID + ", r." + MySQLiteHelper.ID + " AS "
				+ ROOM_ID + ", c." + MySQLiteHelper.ID + " AS "
				+ COMPANY_ID + " FROM " + CompanyHelper.COMPANY_TABLE_NAME
		...
		if (cursor.getCount() >= 1) {
			cursor.moveToFirst();
			Company company = CompanyHelper.getCompanyById(db, cursor.getLong(cursor.getColumnIndex(COMPANY_ID)));
			Building building = BuildingHelper.getBuildingById(db, cursor.getLong(cursor.getColumnIndex(BUILDING_ID)));
			Room room = RoomHelper.getRoomById( cursor.getLong(cursor.getColumnIndex(ROOM_ID)), db);
			...
			//if this happens there is more than one company with the same id in database
			Log.d("studientag", "Fatal Error in getLocationByCompanyId more than one company with the same ID");
			return null;
